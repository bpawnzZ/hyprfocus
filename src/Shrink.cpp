...
void CShrink::onWindowFocus(PHLWINDOW pWindow, HANDLE pHandle) {
  ...
  g_pAnimationManager->createAnimation(1.0f, m_sShrinkAnimation, m_sFocusInAnimConfig, pWindow, AVARDAMAGE_ENTIRE);
  
  static const auto *shrinkPercentage =
      (Hyprlang::FLOAT *const *)(getConfigValue(pHandle, "shrink_percentage")
                                     ->getDataStaticPtr());
  hyprfocus_log(LOG, "Shrink percentage: {}", **shrinkPercentage);
  m_sShrinkAnimation->setValueAndWarp(**shrinkPercentage);

  m_sShrinkAnimation->setUpdateCallback([this, pWindow](Hyprutils::Memory::CWeakPointer<Hyprutils::Animation::CBaseAnimatedVariable> pShrinkAnimation) {
  ...
  m_sShrinkAnimation->setCallbackOnEnd([this, pWindow](Hyprutils::Memory::CWeakPointer<Hyprutils::Animation::CBaseAnimatedVariable> pShrinkAnimation) {
